...
public class ComparingStoreLevelDataInNewUIAndWritingXL {
	
	public enum NewTraditionalKPIs {
		TOTAL, MPA, SOVI, REF, COMM, PRICE, FRESH
	};

	public static HashMap<NewTraditionalKPIs, Integer[]> newTraditionalProperties = new HashMap<NewTraditionalKPIs, Integer[]>();

	static {
		newTraditionalProperties.put(NewTraditionalKPIs.TOTAL, new Integer[] { 6, 7, 8, 14, 6, 9 });
		newTraditionalProperties.put(NewTraditionalKPIs.MPA, new Integer[]   { 16, 17, 18, 4, 7, 0 });
		newTraditionalProperties.put(NewTraditionalKPIs.SOVI, new Integer[]  { 19, 20, 21, 5, 8, 0 });
		newTraditionalProperties.put(NewTraditionalKPIs.REF, new Integer[]   { 22, 23, 24, 6, 9, 0 });
		newTraditionalProperties.put(NewTraditionalKPIs.COMM, new Integer[]  { 25, 26, 27, 7, 10, 0 });
		newTraditionalProperties.put(NewTraditionalKPIs.PRICE, new Integer[] { 28, 29, 30, 8, 11, 0 });
		newTraditionalProperties.put(NewTraditionalKPIs.FRESH, new Integer[] { 31, 32, 33, 9, 12, 0 });
	...
	public void comparingAndWritingData(String writeFilePath, WritableWorkbook writeWorkBook, UIData dataUI,
		...
		int a = 0, b = 1, c = 2, d = 3, e = 4,f = 5, h = 7, /*j = 9,*/ k = 10, l = 11, m = 12, n = 13, o = 14,p = 15;
	...
	private void compareAndWriteForKPI(String storeName, NewTraditionalKPIs kpi, int z1, UIData dataUI, XLData xldata,
		...
		String valueFromXl = xlValues[newTraditionalProperties.get(kpi)[3]];
		valueFromUI = dataUI.getNewTraditionalKPI(kpi, storeName);
        System.out.println("valueFromUI"+" "+valueFromUI);
		
		String kPI = "TOTAL";

		if (kpi.toString().equals(kPI)) {
			Label totalFromUI = new Label(newTraditionalProperties.get(kpi)[0], z1 + 1, Float.toString(valueFromUI));
			writeSheet.addCell(totalFromUI);

			Label iceFromXL = new Label(newTraditionalProperties.get(kpi)[1], z1 + 1, valueFromXl);
			...
			Label diffResult = new Label(newTraditionalProperties.get(kpi)[5], z1 + 1,
					String.valueOf(diffBetweentotalUIAndIceValueXL));
			writeSheet.addCell(diffResult);
			if (diffBetweentotalUIAndIceValueXL >= 0.5) {
				result = new Label(newTraditionalProperties.get(kpi)[2], z1 + 1, "Mismatch");

			} else {
				result = new Label(newTraditionalProperties.get(kpi)[2], z1 + 1, "Match");
			}
			...
			writeSheet.addCell(iceFromXL);
			if (valueFromXl == null) {
				result = new Label(newTraditionalProperties.get(kpi)[2], z1 + 1, "Missing Data in XL");
			} else {
				float diffBetweentotalUIAndIceValueXL = Math.abs(valueFromUI - Float.parseFloat(valueFromXl));
